...
{ 
    ...
    int V[100001];
    int test, aryelement, query, sorch, destination;
    scanf("%d",&test);
    printf("\n");
    for( int i = 1; i<= test; i++ )
    {
        scanf("%d%d",&aryelement, &query);
        for( int j = 1; j <= aryelement; j++  )
        {
            scanf("%d",&V[ j ]);
        }
        printf("Case %d:\n",i);
        ...
        {
            scanf("%d%d",&sorch, &destination);
            int max = 1000001;
            for( int k = sorch; k <= destination; k++ )
            {
                if( V[ k  ] <  max )
                {
                    max = V[ k ];
